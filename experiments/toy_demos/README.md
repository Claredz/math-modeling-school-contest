...
实验，并比较除真实墙钟计时 `runtime_s` 外的所有确定性字段。数值求解字段允许
绝对误差 \(5\times10^{-5}\)、相对误差 \(10^{-9}\) 的跨平台浮点噪声；布尔值、
整数、字符串和结构仍精确比较。文件缺失、JSON 无效、超出容差的确定性字段过期
或目录中出现未声明 JSON 时均返回非零退出码。可用 `--seed` 和 `--output-dir`
显式覆盖默认值。

...
\[
\boldsymbol u_u:[0,T]\rightarrow\mathbb R^2
\quad\text{连续且分段可微},
\]
...
若允许等待，可用 \(\|\dot{\boldsymbol u}_u(t)\|\in\{0,28\}\)；是否允许等待应在场景中显式给出。

### 6.2 无转弯约束的前向可达集

从初始点 \(\boldsymbol u_{u,0}\) 出发，若只要求速度不超过 \(v_u\)，则：

\[
\mathcal R_u(t)
=B(\boldsymbol u_{u,0},v_ut).
\]

若速度模长必须始终等于 \(v_u\) 且不允许盘旋，恰时可达集不是整个圆盘。为了不虚构转弯率，基准候选生成可使用圆盘作为外逼近，再以连续分段直线路径复核。
...
\[
\|\boldsymbol u_u(t)-\boldsymbol b_u\|\le12000,
\]

其中 \(\boldsymbol b_u\) 是任务起点或母舰位置。若团队将其解释为累计航程，本条必须替换为路径长度约束。
...
这些修改需要在团队批准本契约后同步回写 Implementation Plan，当前不直接修改旧计划。
